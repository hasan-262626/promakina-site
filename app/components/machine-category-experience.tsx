...
const configs: Record<CategorySlug, CategoryConfig> = {
  ...
  "toz-toplama-sistemleri": {
    ...
    products: [
      ...
      makeProduct("Torba Filtreler", "Endüstriyel emiş ve filtrasyon hatları için torba filtre çözümleri sunuyoruz.", "Torba filtrede filtre medyası seçimi uzun ömür ve basınç kaybı açısından belirleyicidir.", ["Projeye uygun filtre alanı", "Farklı torba malzemesi", "Pulse jet veya mekanik temizleme", "Modüler gövde tasarımı", "Toz bunker çıkışı", "Servis erişimi"], ["Paketleme ve dolum hatları", "Toz gübre prosesleri", "Kimyasal proses havalandırması", "Mineral toz ürün sistemleri"], ["Rotary valf", "Patlama kapağı", "Basınç sensörü", "Platform", "ATEX opsiyonu", "Fan entegrasyonu"], ["Filtre torbaları", "Kafesler", ...commonSpare]),
      makeProduct("Kartuş Filtreler", "Kompakt alanlarda yüksek verimli filtrasyon için kartuş filtre sistemleri üretiyoruz.", "Kartuş filtrelerde kompakt tasarım avantajı öne çıkar ancak toz tipi ve temizleme davranışı doğru seçilmelidir.", ["Kompakt gövde yapısı", "Farklı kartuş medya seçenekleri", "Pulse jet temizleme sistemi", "Modüler servis erişimi", "Kanal ve fan bağlantıları", "Toz toplama haznesi"], ["Paketleme hatları", "Lokal emiş noktaları", "İnce toz prosesleri", "Kapalı alan filtrasyon sistemleri"], ["Seviye sensörü", "Basınç ölçümü", "ATEX opsiyonu", "Platform", "Rotary valf", "Fan bağlantı paketi"], ["Kartuş filtreler", "Valf setleri", ...commonSpare]),
      makeProduct("Bin Vent Filtreler", "Silo ve bunker havalandırması için bin vent filtre çözümleri sunuyoruz.", "Bin vent filtreler dolum anındaki pik debilere göre seçilmelidir.", ["Kompakt filtre gövdesi", "Silo üstü montaj yapısı", "Pulse jet temizleme opsiyonu", "Dolum debisine uygun filtre alanı", "Kolay servis erişimi", "Rotary valf ve kanal entegrasyonu"], ["Silo sistemleri", "Bunker dolum hatları", "Toz gübre depolama", "Dökme katı ürün besleme"], ["Seviye sensörü", "Basınç ölçümü", "Patlama kapağı", "ATEX opsiyonu", "Rotary valf", "Montaj adaptörleri"], ["Filtre elemanları", "Valf setleri", ...commonSpare]),
    ],
  },
  "paketleme-ve-dolum-sistemleri": {
    ...
    products: [
      ...
      makeProduct("Valfli Torba Dolum", "İnce toz ve hassas ürünler için valfli torba dolum sistemleri sunuyoruz.", "Valfli torba dolumda toz kontrolü ve dolum ağzı tasarımı kritik önemdedir.", ["Valfli torba uyumlu dolum ağzı", "Pnömatik veya mekanik dolum", "Toz kontrollü gövde", "Tartım sistemi entegrasyonu", "Yarı otomatik veya tam otomatik yapı", "Hat sonu taşıma bağlantıları"], ["Toz gübre dolumu", "Mineral toz ürünler", "Kimyasal toz paketleme", "İnce partiküllü ürün hatları"], ["Toz emiş hattı", "Konveyör sistemi", "Checkweigher", "Paletleme entegrasyonu", "Etiketleme sistemi", "Otomasyon paneli"], ["Loadcell", "Pnömatik elemanlar", ...commonSpare]),
      ...
      makeProduct("FFS Torbalama", "Yüksek otomasyonlu torbalama hatları için FFS torbalama sistemleri sunuyoruz.", "FFS torbalama, mekanik ve ambalaj otomasyonunu birlikte çözer.", ["Filmden torba oluşturma sistemi", "Yüksek otomasyonlu dolum", "Tartım ve dozaj entegrasyonu", "Kaynak ve kapama modülü", "Konveyör ve paletleme bağlantısı", "Reçete ve HMI altyapısı"], ["Granül gübre torbalama", "Tohum ve yem hatları", "Dökme katı ürün otomatik paketleme", "Yüksek kapasiteli sevkiyat hatları"], ["Metal dedektör", "Checkweigher", "Etiketleme sistemi", "Paletleme bağlantısı", "Stretch hood entegrasyonu", "Merkezi otomasyon paneli"], ["Çene setleri", "Isıtıcılar", ...commonSpare]),
