...
function SelectInput({
  ...
  return (
    <label className="grid gap-2">
      ...
      <span className="text-xs font-medium text-slate-600">{helperText}</span>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-slate-900 outline-none transition focus:border-blue-300 focus:bg-white">
...
function AutoField({ label, value, unit }: { label: string; value: string; unit?: string }) {
  return <div className="grid gap-2"><span className="text-sm font-medium text-slate-700">{label}</span><span className="text-xs text-slate-500">Standart seçimine göre otomatik gelir</span><div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-slate-900"><span>{value}</span>{unit ? <span className="text-sm text-slate-500">{unit}</span> : null}</div></div>;
}
...
export function ChainSprocketCalculator() {
  ...
  const panel = (title: string, desc: string, form: ReactNode, results: ReactNode, info: string, warnings: ReactNode, extra?: ReactNode) => (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]"><div className="space-y-6">{form}<GuideBox /></div><div className="space-y-6">{results}{warnings}</div></div>
      <InfoBox title="Teknik Bilgi" text={info} />
      {extra}
    </div>
  );
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          <div className="mt-8 space-y-8">
            <div id={`${tabsId}-din8187-panel`} role="tabpanel" aria-labelledby={`${tabsId}-din8187-tab`} hidden={activeTab !== "din8187"}>
              {panel("DIN 8187 Zincir Dişli Hesabı", TABS[0].desc, <div className="space-y-6"><div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Standarttan Otomatik Gelen Alanlar</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><SelectInput label="Adım" value={din.chainCode} onChange={(value) => setDin((s) => ({ ...s, chainCode: value }))} options={DIN_CHAINS.map((i) => ({ value: i.code, label: i.label }))} hint="Zincirin iki pim merkezi arasındaki mesafedir. Hesaplamanın temel ölçüsüdür." tipId="din-pitch" openTip={openTip} setOpenTip={setOpenTip} /><AutoField label="Zincir Kodu" value={selectedDin.code} /><AutoField label="Adım (inç)" value={selectedDin.pitchInch} /><AutoField label="Adım (mm)" value={String(selectedDin.pitchMm)} unit="mm" /><AutoField label="B1" value={selectedDin.b1Upper} unit="mm" /><AutoField label="b1" value={selectedDin.b1Lower} unit="mm" /><AutoField label="B2" value={selectedDin.b2} unit="mm" /><AutoField label="B3" value={selectedDin.b3} unit="mm" /><AutoField label="Makara Çapı" value={selectedDin.rollerDiameter} unit="mm" /><AutoField label="Önerilen Minimum Diş Sayısı" value={String(selectedDin.recommendedMinTeeth)} unit="adet" /></div></div><div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Müşteri Girişleri</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><InputField label="Z / Diş Sayısı" value={din.teeth} onChange={(value) => setDin((s) => ({ ...s, teeth: value }))} unit="adet" hint="Dişli üzerindeki toplam diş sayısıdır. Diş sayısı arttıkça çalışma genellikle daha dengeli olur. Çok düşük diş sayıları titreşim, hız dalgalanması ve aşınmayı artırabilir." helperText="Bu alanı siz doldurun" limitText={`Önerilen min: ${selectedDin.recommendedMinTeeth} diş • Düşük diş sayısı titreşim ve aşınmayı artırabilir`} error={dinCalc.fieldErrors.teeth} tipId="din-teeth" openTip={openTip} setOpenTip={setOpenTip} /><InputField label="Devir Sayısı" value={din.rpm} onChange={(value) => setDin((s) => ({ ...s, rpm: value }))} unit="rpm" hint="Dakikadaki dönüş sayısını ifade eder. Devir arttıkça çevresel hız yükselir ve sistem üzerindeki dinamik etkiler artabilir." helperText="Bu alanı siz doldurun" limitText="0'dan büyük olmalıdır" error={dinCalc.fieldErrors.rpm} tipId="din-rpm" openTip={openTip} setOpenTip={setOpenTip} /><InputField label="Mil Çapı" value={din.shaftDiameter} onChange={(value) => setDin((s) => ({ ...s, shaftDiameter: value }))} unit="mm" hint="Dişlinin bağlanacağı milin dış çapıdır. Mil çapısı, göbek ölçüsünü ve bağlantı emniyetini doğrudan etkiler." helperText="Bu alanı siz doldurun" limitText="Min: 0'dan büyük olmalı • Max: göbek ve dişli geometrisine uygun olmalı" liveText={dinCalc.shaftInfo} error={dinCalc.fieldErrors.shaftDiameter} tipId="din-shaft" openTip={openTip} setOpenTip={setOpenTip} /><InputField label="Göbek Çapı" value={din.hubDiameter} onChange={(value) => setDin((s) => ({ ...s, hubDiameter: value }))} unit="mm" hint="Dişlinin merkez kısmındaki göbek bölgesinin dış çapıdır. Bu ölçü, mil çapısından büyük olmalı ve diş kökü geometrisini bozmayacak sınırlar içinde kalmalıdır." helperText="Bu alanı siz doldurun" limitText="Min: mil çapısından büyük olmalı • Max: önerilen göbek çapısı sınırı" liveText={dinCalc.hubInfo} error={dinCalc.fieldErrors.hubDiameter} tipId="din-hub" openTip={openTip} setOpenTip={setOpenTip} /><InputField label="Opsiyonel Güç" value={din.powerKw} onChange={(value) => setDin((s) => ({ ...s, powerKw: value }))} unit="kW" hint="Girildiğinde çevresel yük veya ön değerlendirme amaçlı ek teknik yorum üretmek için kullanılabilir. Zorunlu değildir." helperText="Girildiğinde çevresel yük tahmini için kullanılabilir" tipId="din-power" openTip={openTip} setOpenTip={setOpenTip} /></div></div></div>, <ResultGrid title="DIN 8187 Zincir Dişli Hesabı" items={[{ label: "Bölüm Dairesi Çapı", value: fmt(dinCalc.d, " mm") }, { label: "Dış Çap", value: fmt(dinCalc.da, " mm") }, { label: "Diş Dibi Çapı", value: fmt(dinCalc.df, " mm") }, { label: "Lineer İlerleme", value: `${fmt(dinCalc.linear, " mm/tur")} / ${fmt(dinCalc.linear / 1000, " m/tur", 4)}` }, { label: "Hat Hızı", value: fmt(dinCalc.v, " m/s", 3) }, { label: "Çalışma Sınıfı", value: dinCalc.speed.label, tone: dinCalc.speed.tone }, { label: "Diş Sayısı Uygunluğu", value: dinCalc.tooth.label, tone: dinCalc.tooth.tone }, { label: "Maksimum Göbek Çapı", value: fmt(dinCalc.hubMax, " mm") }, { label: "Teknik Değerlendirme", value: dinCalc.evaluation }]} />, "DIN 8187 zincir dişli hesabı için seçilen adım, makara çapı ve diş sayısı; bölüm dairesi çapı, zincir hızı ve çalışma kararlılığı üzerinde doğrudan etkilidir.", <WarningBox items={dinCalc.warnings} />)}
            </div>

            <div id={`${tabsId}-asa-panel`} role="tabpanel" aria-labelledby={`${tabsId}-asa-tab`} hidden={activeTab !== "asa"}>
              {panel("ASA Zincir Dişli Hesabı", TABS[1].desc, <div className="space-y-6"><div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Standarttan Otomatik Gelen Alanlar</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><SelectInput label="Zincir Standardı" value={asa.standard} onChange={(value) => setAsa((s) => ({ ...s, standard: value }))} options={ASA_CHAINS.map((i) => ({ value: i.code, label: i.label }))} hint="Standart seçildiğinde ilgili zincir verileri otomatik gelir." /><AutoField label="Adım (mm)" value={String(selectedAsa.pitchMm)} unit="mm" /><AutoField label="Makara Çapı" value={selectedAsa.rollerDiameter} unit="mm" /><AutoField label="Genişlik" value={selectedAsa.width} unit="mm" /><AutoField label="Önerilen Minimum Diş Sayısı" value={String(selectedAsa.recommendedMinTeeth)} unit="adet" /></div></div><div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Müşteri Girişleri</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><InputField label="Z / Diş Sayısı" value={asa.teeth} onChange={(value) => setAsa((s) => ({ ...s, teeth: value }))} unit="adet" hint="Dişli üzerindeki toplam diş adedidir." /><InputField label="Devir Sayısı" value={asa.rpm} onChange={(value) => setAsa((s) => ({ ...s, rpm: value }))} unit="rpm" hint="Sisteme giriş yapan döndürücü elemanın dakika başına devir sayısıdır." /><InputField label="Mil Çapı" value={asa.shaftDiameter} onChange={(value) => setAsa((s) => ({ ...s, shaftDiameter: value }))} unit="mm" hint="Dişlinin bağlanacağı milin dış çapıdır." /><InputField label="Göbek Çapı" value={asa.hubDiameter} onChange={(value) => setAsa((s) => ({ ...s, hubDiameter: value }))} unit="mm" hint="Göbek ölçüsü mil ve dişli geometrisine uygun olmalıdır." /></div></div></div>, <ResultGrid title="ASA Zincir Dişli Hesabı" items={[{ label: "Bölüm Dairesi Çapı", value: fmt(asaCalc.d, " mm") }, { label: "Dış Çap", value: fmt(asaCalc.da, " mm") }, { label: "Diş Dibi Çapı", value: fmt(asaCalc.df, " mm") }, { label: "Lineer İlerleme", value: `${fmt(asaCalc.linear, " mm/tur")} / ${fmt(asaCalc.linear / 1000, " m/tur", 4)}` }, { label: "Hat Hızı", value: fmt(asaCalc.v, " m/s", 3) }, { label: "Çalışma Sınıfı", value: asaCalc.speed.label, tone: asaCalc.speed.tone }, { label: "Diş Sayısı Uygunluğu", value: asaCalc.tooth.label, tone: asaCalc.tooth.tone }, { label: "Maksimum Göbek Çapı", value: fmt(asaCalc.hubMax, " mm") }, { label: "Teknik Değerlendirme", value: asaCalc.warnings[0] ?? "ASA zincir dişli hesabı için seçilen değerler uygundur." }]} />, "ASA zincir dişli hesabı, seçilen standarda göre zincir verilerini otomatik getirir ve kullanıcıyı manuel standart veri girişinden kurtarır.", asaCalc.warnings.length ? <InfoBox title="Teknik Uyarılar" text={asaCalc.warnings.join(" ")} /> : null)}
            </div>

            <div id={`${tabsId}-conveyor-panel`} role="tabpanel" aria-labelledby={`${tabsId}-conveyor-tab`} hidden={activeTab !== "conveyor"}>
              {panel("Konveyör Zincir Dişlileri", TABS[2].desc, <div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Müşteri Girişleri</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><InputField label="Adım (mm)" value={conveyor.pitch} onChange={(value) => setConveyor((s) => ({ ...s, pitch: value }))} unit="mm" hint="Zincirin iki pim merkezi arasındaki mesafedir." /><InputField label="Z / Diş Sayısı" value={conveyor.teeth} onChange={(value) => setConveyor((s) => ({ ...s, teeth: value }))} unit="adet" hint="Dişli üzerindeki toplam diş adedidir." /><InputField label="Tahrik Devri" value={conveyor.driveRpm} onChange={(value) => setConveyor((s) => ({ ...s, driveRpm: value }))} unit="rpm" hint="Sisteme giriş yapan devirdir." /><InputField label="Hat Uzunluğu" value={conveyor.lineLength} onChange={(value) => setConveyor((s) => ({ ...s, lineLength: value }))} unit="m" hint="Taşıma hattının toplam boyudur." /><InputField label="Taşıma Kapasitesi" value={conveyor.capacity} onChange={(value) => setConveyor((s) => ({ ...s, capacity: value }))} unit="kg/saat" hint="Opsiyonel kapasite bilgisidir." /><InputField label="Çalışma Süresi" value={conveyor.workingHours} onChange={(value) => setConveyor((s) => ({ ...s, workingHours: value }))} unit="saat/gün" hint="Opsiyonel çalışma süresidir." /><SelectInput label="Sistem Tipi" value={conveyor.systemType} onChange={(value) => setConveyor((s) => ({ ...s, systemType: value }))} options={["Yatay Konveyör", "Eğimli Konveyör", "Paletli Sistem"].map((v) => ({ value: v, label: v }))} /><SelectInput label="Yük Tipi" value={conveyor.loadType} onChange={(value) => setConveyor((s) => ({ ...s, loadType: value }))} options={["Hafif Yük", "Orta Yük", "Ağır Yük"].map((v) => ({ value: v, label: v }))} /></div></div>, <ResultGrid title="Konveyör Zincir Dişlileri" items={[{ label: "Bölüm Dairesi Çapı", value: fmt(conveyorCalc.d, " mm") }, { label: "Dış Çap", value: fmt(conveyorCalc.da, " mm") }, { label: "Diş Dibi Çapı", value: fmt(conveyorCalc.df, " mm") }, { label: "Hat Hızı", value: fmt(conveyorCalc.v, " m/s", 3) }, { label: "Lineer İlerleme", value: `${fmt(conveyorCalc.linear, " mm/tur")} / ${fmt(conveyorCalc.linear / 1000, " m/tur", 4)}` }, { label: "Çalışma Sınıfı", value: conveyorCalc.speed.label, tone: conveyorCalc.speed.tone }, { label: "Diş Sayısı Uygunluğu", value: conveyorCalc.tooth.label, tone: conveyorCalc.tooth.tone }, { label: "Teknik Değerlendirme", value: conveyorCalc.warnings[0] ?? "Konveyör sistemlerinde zincir hızı ve dişli çapı taşıma verimliliğini doğrudan etkiler." }]} />, "Konveyör zincir dişlileri için düşük diş sayısı, aşırı hız ve yetersiz kombinasyonlar sistem ömrünü kısaltabilir.", conveyorCalc.warnings.length ? <InfoBox title="Teknik Uyarılar" text={conveyorCalc.warnings.join(" ")} /> : null)}
            </div>

            <div id={`${tabsId}-customPitch-panel`} role="tabpanel" aria-labelledby={`${tabsId}-customPitch-tab`} hidden={activeTab !== "customPitch"}>
              {panel("Özel Adımlı Zincir Dişliler", TABS[3].desc, <div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Müşteri Girişleri</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><InputField label="Özel Adım (mm)" value={customPitch.pitch} onChange={(value) => setCustomPitch((s) => ({ ...s, pitch: value }))} unit="mm" hint="Standart dışı zincirin iki pim merkezi arasındaki mesafedir." /><InputField label="Z / Diş Sayısı" value={customPitch.teeth} onChange={(value) => setCustomPitch((s) => ({ ...s, teeth: value }))} unit="adet" hint="Dişli üzerindeki toplam diş adedidir." /><InputField label="Mil Çapı" value={customPitch.shaftDiameter} onChange={(value) => setCustomPitch((s) => ({ ...s, shaftDiameter: value }))} unit="mm" hint="Dişlinin bağlanacağı milin dış çapıdır." /><InputField label="Göbek Çapı" value={customPitch.hubDiameter} onChange={(value) => setCustomPitch((s) => ({ ...s, hubDiameter: value }))} unit="mm" hint="Göbek ölçüsü mil ve dişli geometrisine uygun olmalıdır." /></div></div>, <ResultGrid title="Özel Adımlı Zincir Dişliler" items={[{ label: "Bölüm Dairesi Çapı", value: fmt(customCalc.d, " mm") }, { label: "Dış Çap", value: fmt(customCalc.da, " mm") }, { label: "Diş Dibi Çapı", value: fmt(customCalc.df, " mm") }, { label: "Lineer İlerleme", value: `${fmt(customCalc.linear, " mm/tur")} / ${fmt(customCalc.linear / 1000, " m/tur", 4)}` }, { label: "Diş Sayısı Uygunluğu", value: customCalc.tooth.label, tone: customCalc.tooth.tone }, { label: "Standart Yakınlık", value: customCalc.special ? `${customCalc.nearest.label} standardına yakındır / özel imalat doğrulaması önerilir` : "Standart adıma çok yakın", tone: customCalc.special ? "warning" : "success" }, { label: "Maksimum Göbek Çapı", value: fmt(customCalc.hubMax, " mm") }, { label: "Teknik Değerlendirme", value: customCalc.warnings[0] ?? "Standart dışı üretim doğrulaması önerilir." }]} />, "Özel adımlı zincir dişlilerde standart dışı üretim gerekliliği ve tolerans kontrolü kritik öneme sahiptir.", customCalc.warnings.length ? <InfoBox title="Teknik Uyarılar" text={customCalc.warnings.join(" ")} /> : null)}
            </div>

            <div id={`${tabsId}-ratio-panel`} role="tabpanel" aria-labelledby={`${tabsId}-ratio-tab`} hidden={activeTab !== "ratio"}>
              {panel("İletim / Devir Oranı Hesabı İkili", TABS[4].desc, <div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Müşteri Girişleri</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><InputField label="Adım (mm)" value={ratio.pitch} onChange={(value) => setRatio((s) => ({ ...s, pitch: value }))} unit="mm" hint="Zincirin iki pim merkezi arasındaki mesafedir. Hesaplamanın temel ölçüsüdür." /><InputField label="Giriş Devri / Redüktör Devri" value={ratio.inputRpm} onChange={(value) => setRatio((s) => ({ ...s, inputRpm: value }))} unit="dev/dk" hint="Sisteme giriş yapan döndürücü elemanın dakika başına devir sayısıdır." /><InputField label="Z1" value={ratio.z1} onChange={(value) => setRatio((s) => ({ ...s, z1: value }))} unit="adet" hint="Z1 = döndüren veya büyük dişli olarak kullanılan diş sayısıdır." /><InputField label="Z2" value={ratio.z2} onChange={(value) => setRatio((s) => ({ ...s, z2: value }))} unit="adet" hint="Z2 = döndürülen veya küçük dişli olarak kullanılan diş sayısıdır." /></div></div>, <ResultGrid title="Zincir Dişli İletim Oranı Detay" items={[{ label: "İletim Oranı (i)", value: `i = ${fmt(ratioCalc.i, "", 3)}` }, { label: "Z2'nin Devri", value: fmt(ratioCalc.n2, " dev/dk") }, { label: "Sistem Tipi", value: ratioCalc.system, tone: ratioCalc.system === "Hız Düşürücü" ? "warning" : "success" }, { label: "Devir Değişim Oranı", value: fmt(ratioCalc.change, " %", 2), tone: ratioCalc.change < 0 ? "warning" : "success" }, { label: "Z1 bir tur döndüğünde Z2'nin turu", value: `${fmt(ratioCalc.oneTurn, "", 3)} tur` }, { label: "Zincir Hızı", value: n(ratio.pitch) > 0 ? fmt(ratioCalc.v, " m/s", 3) : "Sonuçlar burada görüntülenecektir" }, { label: "Diş Sayısı Uygunluğu", value: ratioCalc.tooth.label, tone: ratioCalc.tooth.tone }, { label: "Teknik Değerlendirme", value: ratioCalc.system === "Hız Düşürücü" ? "Z2 diş sayısı büyüdükçe çıkış devri düşer." : ratioCalc.system === "Hız Artırıcı" ? "Z2 diş sayısı küçüldükçe çıkış devri artar." : "Sistem 1:1 iletim mantığında çalışır." }]} />, "Zincir dişli iletim oranı hesabında döndüren ve döndürülen dişli arasındaki diş sayısı ilişkisi, çıkış devrini doğrudan belirler. Doğru oran seçimi; hız, tork ve çalışma stabilitesi açısından kritik önem taşır.", <InfoBox title="Teknik Uyarılar" text={ratioCalc.tooth.label} />, <InfoBox title="İletim Oranı Nasıl Yorumlanır?" text="Z2 diş sayısı büyüdükçe çıkış devri düşer. Z2 diş sayısı küçüldükçe çıkış devri artar. Daha yüksek oranlar genellikle daha düşük hız ve daha yüksek tork sağlar." />)}
            </div>

            <div id={`${tabsId}-chainLength-panel`} role="tabpanel" aria-labelledby={`${tabsId}-chainLength-tab`} hidden={activeTab !== "chainLength"}>
              {panel("İkili Sistem Zincir Bakla Boy Hesabı", TABS[5].desc, <div className="space-y-6"><div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Müşteri Girişleri</h3><div className="mt-5 grid gap-4 sm:grid-cols-2"><InputField label="Adım (mm)" value={chainLength.pitch} onChange={(value) => setChainLength((s) => ({ ...s, pitch: value }))} unit="mm" hint="Zincirin iki pim merkezi arasındaki mesafedir." /><InputField label="Büyük Dişli (Z1)" value={chainLength.z1} onChange={(value) => setChainLength((s) => ({ ...s, z1: value }))} unit="adet" hint="Z1 = büyük dişli" /><InputField label="Küçük Dişli (Z2)" value={chainLength.z2} onChange={(value) => setChainLength((s) => ({ ...s, z2: value }))} unit="adet" hint="Z2 = küçük dişli" /><InputField label="Eksenler Arası (CE)" value={chainLength.centerDistance} onChange={(value) => setChainLength((s) => ({ ...s, centerDistance: value }))} unit="mm" hint="İki dişli merkezinin teorik mesafesidir." /><AutoField label="L" value={fmt(chainLengthCalc.l, "", 3)} /><AutoField label="Zincir Uzunluğu mm" value={fmt(chainLengthCalc.lengthMm, " mm")} /><AutoField label="Bakla Sayısı Üst Değer ~" value={fmt(chainLengthCalc.rounded, "", 0)} /><AutoField label="Zincire En Uygun Eksen Mesafesi (CL)" value={fmt(chainLengthCalc.cl, " mm")} /></div></div></div>, <ResultGrid title="İkili Sistem Zincir Bakla Boy Hesabı" items={[{ label: "Teorik Bakla Sayısı (L)", value: fmt(chainLengthCalc.l, "", 3) }, { label: "Bakla Sayısı Üst Değer ~", value: fmt(chainLengthCalc.rounded, " adet", 0) }, { label: "Zincir Uzunluğu (mm)", value: fmt(chainLengthCalc.lengthMm, " mm") }, { label: "Zincir Uzunluğu (m)", value: fmt(chainLengthCalc.lengthM, " m", 4) }, { label: "Girilen Eksen Mesafesi (CE)", value: fmt(chainLengthCalc.ce, " mm") }, { label: "Zincire En Uygun Eksen Mesafesi (CL)", value: fmt(chainLengthCalc.cl, " mm") }, { label: "Merkez Mesafesi Farkı", value: fmt(chainLengthCalc.delta, " mm", 2), tone: chainLengthCalc.fit.tone }, { label: "Uygunluk Değerlendirmesi", value: chainLengthCalc.fit.label, tone: chainLengthCalc.fit.tone }, { label: "Teknik Not", value: "Büyük dişli / küçük dişli farkı arttıkça zincir boyu ve merkez mesafesi hassasiyeti artar." }]} />, "İkili zincir dişli sistemlerinde doğru bakla sayısı ve uygun eksen mesafesi, sistemin sessiz çalışması ve gergi ayarının sağlıklı yapılabilmesi için kritik öneme sahiptir. Teorik merkez mesafesi ile gerçek zincir boyuna göre oluşan fark mutlaka dikkate alınmalıdır.", ([chainLengthCalc.swapped ? "Büyük dişli ve küçük dişli değerleri otomatik sıralandı." : "", Math.abs(chainLengthCalc.delta) > 5 ? "Bu zincir boyu için eksen mesafesi revize edilmelidir." : ""].filter(Boolean).length ? <InfoBox title="Teknik Uyarılar" text={[chainLengthCalc.swapped ? "Büyük dişli ve küçük dişli değerleri otomatik sıralandı." : "", Math.abs(chainLengthCalc.delta) > 5 ? "Bu zincir boyu için eksen mesafesi revize edilmelidir." : ""].filter(Boolean).join(" ")} /> : null), <InfoBox title="Bakla Sayısı Neden Çift Seçilir?" text="Rulo zincir sistemlerinde zincir bağlantı yapısı nedeniyle bakla sayısı çoğu uygulamada çift sayıya yuvarlanır. Bu nedenle teorik sonuç ile uygulanabilir zincir boyu arasında küçük farklar oluşabilir." />)}
            </div>
