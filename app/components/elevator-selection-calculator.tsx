...
export function ElevatorSelectionCalculator() {
  ...
  const [accessories, setAccessories] = useState<string[]>(["Hiz sensörü", "Geri dönüs önleyici", "Bakim kapagi"]);
  ...
  const model = useMemo(() => {
    ...
    const chosenBeltType = temperatureBand === "Sicak" || temperatureBand === "Çok sicak" ? "Isiya dayanimli bant" : usage === "Gida ortami" ? "Gida tipi bant" : usage === "Kimyasal ortam" ? "Kimyasala dayanimli bant" : beltType;
    ...
    const selectedChainMaterial = corrosion === "Korozif" || corrosion === "Çok korozif" ? "Paslanmaz" : grain === "Asindirici" ? "Asinma dayanimli" : chainMaterial;
        ...
        <div className="grid gap-6 xl:grid-cols-[1.04fr_0.96fr]">
          <div className="space-y-6">
            <div className="rounded-[32px] border border-blue-100 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
              <h2 className="text-2xl font-semibold text-slate-950">Müsteriden Alinacak Bilgiler</h2>
              <p className="mt-3 text-sm leading-7 text-slate-600">Gerekli seçim ve girisleri bu bölümde tamamlayin. Sistem elevatör tipi, kova seçimi, hiz ve ön mekanik önerileri anlik olarak olusturur.</p>
              ...
              <div className="mt-6 grid gap-5 md:grid-cols-2">
                ...
                <ToolkitInput label="Proje adi" value={projectName} onChange={setProjectName} helperText="Teknik rapor basligi" limitText="Opsiyoneldir" tip="Proje veya hat ismi olarak rapora tasinir." tipId="elevator-project" openTip={openTip} setOpenTip={setOpenTip} />
              ...
              <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div className="mt-4 grid gap-5 md:grid-cols-2">
                  <ToolkitSelect label="Tasinacak ürün" value={product} onChange={(value) => { setProduct(value); setDensity(String(PRODUCT_DENSITIES[value] ?? 900)); }} options={[...PRODUCT_OPTIONS]} helperText="Ürün tipi yogunlugu ve mekanik tavsiyeleri etkiler" limitText="Yogunluk otomatik gelir, isterseniz degistirebilirsiniz" tip="Tasinacak ürün tipi; yogunluk, kova seçimi, elevatör tipi, hiz ve mekanik yapi üzerinde dogrudan etkilidir." tipId="elevator-product" openTip={openTip} setOpenTip={setOpenTip} />
                  ...
                  <ToolkitSelect label="Kullanim yeri" value={usage} onChange={setUsage} options={[...USAGE_OPTIONS]} helperText="Ortam kosullari koruma seviyesini belirler" limitText="Kapak, boya, galvaniz ve paslanmaz önerileri otomatik olusur" tip="Çalisma ortami; gövde kaplamasi, sizdirmazlik, bakim kapaklari ve koruma detaylarini etkiler." tipId="elevator-usage" openTip={openTip} setOpenTip={setOpenTip} />
                </div>
              </div>
              ...
              <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div className="mt-4 grid gap-5 md:grid-cols-2">
                  <ToolkitInput label="Malzemeyi kaç metre yukari çikaracak?" value={liftHeight} onChange={setLiftHeight} unit="m" error={errors.liftHeight} helperText="Tasima yüksekligi" limitText="Toplam gövde boyu ve güç hesabini etkiler" tip="Elevatörün tasima yüksekligidir. Toplam gövde boyu, zincir/bant boyu ve güç hesabini etkiler." tipId="elevator-height" openTip={openTip} setOpenTip={setOpenTip} />
                  <ToolkitSelect label="Ürün giris sekli" value={inletType} onChange={setInletType} options={[...INLET_OPTIONS]} helperText="Besleme karakteri ayak tasarimini etkiler" limitText="Düzensiz besleme daha emniyetli giris ister" tip="Besleme tipi, ayak yapisi ve doluluk davranisini etkiler." tipId="elevator-inlet" openTip={openTip} setOpenTip={setOpenTip} />
                  <ToolkitSelect label="Ürün çikis sekli" value={outletType} onChange={setOutletType} options={[...OUTLET_OPTIONS]} helperText="Kafa tasarimini etkiler" limitText="Bosaltma yönüne göre kapak ve oluk degisebilir" tip="Bosaltma yönü ve çikis düzeni kafa tasarimini etkiler." tipId="elevator-outlet" openTip={openTip} setOpenTip={setOpenTip} />
              ...
              <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5">
                ...
                {!plasticAllowed ? <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-7 text-amber-900">Sicak ürün nedeniyle plastik kova kullanimi uygun degildir.</div> : null}
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <ToolkitSelect label="Kova araligi" value={spacingMode} onChange={setSpacingMode} options={[...SPACING_MODE_OPTIONS]} helperText="Sistem veya manuel deger" limitText="Kovalar arasindaki mesafe kapasiteyi dogrudan etkiler" tip="Kovalar arasindaki mesafedir. Kapasiteyi dogrudan etkiler." tipId="elevator-spacing-mode" openTip={openTip} setOpenTip={setOpenTip} />
            ...
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <SuggestionCard title="Plastik kova uygun mu?" value={model.plasticAllowed ? "Uygun olabilir" : "Uygun degil"} tone={model.plasticAllowed ? "slate" : "amber"} />
          ...
          <div className="space-y-6 xl:sticky xl:top-6 xl:self-start">
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="text-2xl font-semibold text-slate-950">Müsteri Sonuç Ekrani</h2>
            ...
            <ResultSection title="E) Gövde ve Mekanik" items={[
              ...
              { label: "Bakim kapagi ihtiyaci", value: accessories.includes("Bakim kapagi") ? "Seçildi" : "Önerilir" },
            ...
            <ToolkitLead title="Teknik özet raporu hazir" text={`Müsteri: ${customerName || "Belirtilmedi"} | Proje: ${projectName || "Belirtilmedi"} | Tarih: ${dateValue || "Belirtilmedi"}. Bu özet PDF olarak kaydedilebilir ve müsteri sunumunda kullanilabilir.`} />
          </div>
        </div>
      </div>
    </section>
