...
export function SectorInquiryForm({ sectorTitle, config }: SectorInquiryFormProps) {
  ...
  const [dynamicState, setDynamicState] = useState<Record<string, string>>(() =>
    buildInitialDynamicState(config),
  );

  const allMessageLines = useMemo(() => {
    const lines = [
      `Merhaba, ${sectorTitle} için bilgi / teklif talep ediyorum.`,
      "",
  ...
  function submitTo(target: "whatsapp" | "mail") {
    if (!formRef.current?.reportValidity() || !commonState.consent) {
      return;
    }

    if (target === "whatsapp") {
      window.open(
        `https://wa.me/905380631163?text=${encodeURIComponent(allMessageLines)}`,
        "_blank",
        "noopener,noreferrer",
      );
      return;
    ...
    window.location.href = `mailto:info@promakina.com.tr?subject=${encodeURIComponent(
      `${sectorTitle} - Teklif / Teknik Görüşme`,
    ...
    <form ref={formRef} className="p-6 sm:p-8">
      <div className="grid gap-5 md:grid-cols-2">
          ...
          <label key={field.id} className="space-y-2">
            ...
            {field.type === "select" ? (
              <select
                ...
                onChange={(event) =>
                  setDynamicState((current) => ({ ...current, [field.id]: event.target.value }))
                }
                className="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500"
              >
        ...
        <label className="space-y-2">
          ...
          <select
            required
            value={commonState.automationLevel}
            onChange={(event) => setCommonState((current) => ({ ...current, automationLevel: event.target.value }))}
            className="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500"
          >
        ...
        <label className="space-y-2 md:col-span-2">
          ...
          <select
            required
            value={commonState.installationModel}
            onChange={(event) =>
              setCommonState((current) => ({ ...current, installationModel: event.target.value }))
            }
            className="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500"
          >
