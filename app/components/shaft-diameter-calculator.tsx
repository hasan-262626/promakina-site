      ...
      <div className="site-container">
        ...
        {activeTab !== "reference" ? (
            ...
            <div className="space-y-6">
              <div className="rounded-3xl border border-blue-100 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
                ...
                <div className="mt-6 grid gap-5 md:grid-cols-2">
                  ...
                  <ToolkitSelect label="Malzeme Türü" value={material} onChange={setMaterial} options={Object.keys(MATERIALS)} helperText="Bu alanı siz seçin" limitText="İzin verilen kayma gerilmesi buna göre gelir" tip="Mil malzemesine göre izin verilen kayma gerilmesi değişir." tipId="shaft-material" openTip={openTip} setOpenTip={setOpenTip} />
                  <ToolkitInput label="Emniyet Katsayısı" value={safety} onChange={setSafety} helperText="Bu alanı siz doldurun" limitText="1’den büyük olmalıdır" tip="Emniyet katsayısı arttıkça önerilen mil çapı büyür." tipId="shaft-safety" openTip={openTip} setOpenTip={setOpenTip} />
                  <ToolkitSelect label="Mil Tipi" value={shaftType} onChange={setShaftType} options={["Dolu Mil", "Boru Mil"]} helperText="Bu alanı siz seçin" limitText="Kesit tipini belirler" tip="Milin dolu veya boru tipinde olduğunu belirtir." tipId="shaft-type" openTip={openTip} setOpenTip={setOpenTip} />
                  {shaftType === "Boru Mil" ? <ToolkitInput label="İç Çap" value={innerDiameter} onChange={setInnerDiameter} unit="mm" helperText="Bu alanı siz doldurun" limitText="Boru mil için gereklidir" tip="Boru milin iç çapıdır. Çok yüksek değerler burulma dayanımını düşürebilir." tipId="shaft-inner" openTip={openTip} setOpenTip={setOpenTip} /> : null}
                  {activeTab === "combined" ? <ToolkitInput label="Eğilme Katsayısı" value={bendingFactor} onChange={setBendingFactor} helperText="Bu alanı siz doldurun" limitText="1 ve üzeri önerilir" tip="Eğilme etkisini sade ön hesapla büyütmek için kullanılır." tipId="shaft-bending" openTip={openTip} setOpenTip={setOpenTip} /> : null}
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  ...
                  {activeTab !== "torsion" ? <ToolkitResult label="Eğilme + Burulma Çapı" value={`${fmt(model.combined, 2)} mm`} /> : null}
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            ...
            <ToolkitInfo title="Katı ve boru mil farkı nedir?" text="Katı mil daha sade ve rijit yapı sunarken, boru mil aynı dış çapa göre daha hafif olabilir. Ancak iç çap büyüdükçe dayanım azalır." />
          </div>
        )}
      </div>
    </section>
