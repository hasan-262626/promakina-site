...
import type { HomeMachineGroupCard } from "../home-machine-groups-data";
 ...
 */
export type HomeMachineGroupsContent = {
  heading: string;
  description: string;
  cards: HomeMachineGroupCard[];
};

export function HomeMachineGroupsSection({ content }: { content?: HomeMachineGroupsContent }) {
  const router = useRouter();
  const heading = content?.heading ?? "Makinalar & Ekipmanlar";
  const description =
    content?.description ??
  ...
  const cards = content?.cards ?? homeMachineGroupCards;
