            ...
            <div id={`${tabsId}-geometry-panel`} role="tabpanel" aria-labelledby={`${tabsId}-geometry-tab`} hidden={activeTab !== "geometry"} className="space-y-6">
              ...
              <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                <div className="space-y-6">
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6"><h3 className="text-lg font-semibold text-slate-950">Otomatik Gelen / Hesaplanan Alanlar</h3><div className="mt-5 grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5"><AutoField label="İç Çember Bölüm Dairesi (Ødo1)" value={fmt(geometryResult.d1, " mm")} /><AutoField label="İç Çember Diş Üstü Çapı" value={fmt(geometryResult.da1, " mm")} /><AutoField label="İç Çember Diş Dibi Çapı (Ødf1)" value={fmt(geometryResult.df1, " mm")} /><AutoField label="İç Çember Temel Daire (Ødg1)" value={fmt(geometryResult.db1, " mm")} /><AutoField label="Pinyon Bölüm Dairesi (Ødo2)" value={fmt(geometryResult.d2, " mm")} /><AutoField label="Pinyon Dış Çapı (Øda2)" value={fmt(geometryResult.da2, " mm")} /><AutoField label="Pinyon Diş Dibi Çapı (Ødf2)" value={fmt(geometryResult.df2, " mm")} /><AutoField label="Pinyon Temel Daire (Ødg2)" value={fmt(geometryResult.db2, " mm")} /><AutoField label="Eksenler Arası Mesafe (E)" value={fmt(geometryResult.center, " mm")} /><AutoField label="Diş Derinliği (H)" value={fmt(geometryResult.depth, " mm")} /><AutoField label="Toplam Açı°" value={geometryResult.totalAngle} /><AutoField label="Kosinüs Değeri" value={geometryResult.cosValue} /></div></div>
                  <GuideBox />
                </div>
