import Image from "next/image";
import Link from "next/link";
import type { HomeServiceCard as HomeServiceCardData } from "../home-services-data";
import { homeServiceCards, homeServiceTags } from "../home-services-data";
import { trText } from "../lib/tr-text";

const visibleHomeServiceCards = homeServiceCards.filter(
  (card) => trText(card.title) !== "Pilot Tesis ve Proses Test Çalışmaları",
);

/**
 * Bölüm içerik modeli. Verilmezse Türkçe varsayılanı kullanılır; yabancı diller
 * aynı bileşeni kendi kart verileriyle besler. Kart tasarımı ortaktır.
 */
export type HomeServicesContent = {
  heading: string;
  description: string;
  tags: { label: string; href: string }[];
  cards: HomeServiceCardData[];
  detailLabel: string;
};

function ServiceCard({ card, detailLabel }: { card: HomeServiceCardData; detailLabel: string }) {
  return (
    <article className="group flex h-full flex-col rounded-[28px] border border-slate-200/80 bg-white p-6 shadow-[0_14px_40px_rgba(15,23,42,0.05)] transition duration-200 hover:-translate-y-1 hover:border-[#278DC0]/16 hover:shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
      <div className="relative h-16 w-16 overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-[0_8px_24px_rgba(15,23,42,0.08)]">
        <Image
          src={card.image}
          alt={trText(card.imageAlt)}
          fill
          sizes="64px"
          className="object-cover object-center"
        />
      </div>
      <h3 className="mt-5 text-xl font-semibold tracking-tight text-slate-950">{trText(card.title)}</h3>
      <p className="mt-3 flex-1 text-sm leading-7 text-slate-600 sm:text-[15px]">{trText(card.description)}</p>
      <Link
        href={card.href}
        className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#278DC0] transition hover:text-[#154764] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#278DC0]"
      >
        {detailLabel}
        <span aria-hidden="true" className="inline-block rtl:-scale-x-100">-&gt;</span>
      </Link>
    </article>
  );
}

export function HomeServicesSection({ content }: { content?: HomeServicesContent }) {
  const heading = content?.heading ?? "HİZMETLERİMİZ";
  const description =
    content?.description ??
    "Pro Makina; gübre ve granülasyon, kompost ve organik atık, geri dönüşüm, atık su çamuru, biyogaz, madencilik ve dökme katı malzeme işleme alanlarında projeye özel mühendislik ve uygulama çözümleri sunar. İhtiyaç analizinden proses tasarımına, makina imalatından saha kurulumuna ve devreye almaya kadar tüm süreçler tek merkezden planlanır.";
  const tags = content?.tags ?? homeServiceTags;
  const cards = content?.cards ?? visibleHomeServiceCards;
  const detailLabel = content?.detailLabel ?? "Detayları Gör";

  return (
    <section
      id="hizmetler"
      className="bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] py-10 md:py-14 lg:py-16"
      aria-labelledby="home-services-title"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        <div className="max-w-4xl">
          <h2
            id="home-services-title"
            className="text-3xl font-semibold tracking-tight text-[#278DC0] sm:text-4xl"
          >
            {trText(heading)}
          </h2>
          <p className="mt-4 max-w-[1180px] text-[15px] leading-[1.7] text-slate-900 sm:text-base">
            {trText(description)}
          </p>
        </div>

        <div className="mt-6 md:mt-8 flex flex-wrap gap-3">
          {tags.map((tag) => (
            <Link
              key={tag.href}
              href={tag.href}
              aria-label={trText(tag.label)}
              className="inline-flex rounded-full border border-[#278DC0]/16 bg-white px-4 py-2 text-sm font-medium text-[#278DC0] shadow-sm transition hover:border-[#278DC0] hover:text-[#154764] hover:shadow-[0_8px_22px_rgba(15,23,42,0.06)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#278DC0]"
            >
              {trText(tag.label)}
            </Link>
          ))}
        </div>

        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {cards.map((card) => (
            <ServiceCard key={card.title} card={card} detailLabel={detailLabel} />
          ))}
        </div>
      </div>
    </section>
  );
}
