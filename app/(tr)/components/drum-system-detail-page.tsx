...
const defaultDrumBlogVisuals: Record<string, { src: string; alt: string }> = {
  "kurutma-tamburu-hesaplama": {
    ...
    alt: "Kurutma tamburu hesaplama ve kapasite değerlendirmesi",
  },
  "rotary-dryer-design": {
  ...
  "endustriyel-kurutma-sistemleri": {
    src: "/images/tambur kurutma/tamkurutma10.jpg",
  ...
  "kurutma-tamburu-tasarim-kriterleri": {
    src: "/images/tambur kurutma/tamkurutma11.jpg",
    alt: "Kurutma tamburu tasarım kriterleri",
  },
  "kurutma-tamburunda-brulor-secimi": {
    src: "/images/tambur kurutma/tamkurutma12.jpg",
  ...
  "perlit-kurutma-tamburu": {
    src: "/images/tambur kurutma/tamkurutma8.jpg",
    alt: "Perlit kurutma tamburu ve toz toplama sistemi",
  },
  "kalsit-kurutma-tamburu": {
    src: "/images/tambur kurutma/tamkurutma9.jpg",
    alt: "Kalsit kurutma tamburu ve öğütme öncesi nem alma hattı",
  },
  ...
  "bentonit-kurutma-tamburu": {
    src: "/images/tambur kurutma/tamkurutma5.jpg",
    alt: "Bentonit kurutma tamburu ve eleme hattı",
  },
  "kuvars-kumu-kurutma-tamburu": {
    src: "/images/tambur kurutma/tamkurutma6.jpg",
    alt: "Kuvars kumu kurutma tamburu ve sınıflandırma hattı",
  },
  "feldspat-kurutma-tamburu": {
    src: "/images/tambur kurutma/tamkurutma7.jpg",
    alt: "Feldspat kurutma tamburu ve öğütme hattı beslemesi",
  },
};
...
const dryingBlogClusterSlugs = new Set([
  "kurutma-tamburu-hesaplama",
  "rotary-dryer-design",
  "endustriyel-kurutma-sistemleri",
  "tambur-kurutucu-nasil-calisir",
...
function isDryingBlogClusterPage(slug: string) {
  return dryingBlogClusterSlugs.has(slug);
}

function getProjectDataCards(page: DrumSystemDetailPageData) {
  const rows = page.selectionCriteria.rows.slice(0, 8);

  if (rows.length >= 6) {
    return rows.map(([label, value]) => ({
      label,
      value: value || "Proje verisine göre belirlenir",
    }));
  }

  return [
    { label: "Ürün", value: page.title },
    { label: "Giriş nemi", value: "Laboratuvar analizine göre belirlenir" },
    { label: "Hedef çıkış nemi", value: "Kullanım alanına göre belirlenir" },
    { label: "Saatlik kapasite", value: "Ton/saat bazında hesaplanır" },
    { label: "Ürün formu", value: "Tane boyutu veya reçeteye göre belirlenir" },
    { label: "Yakıt tipi", value: "Proje altyapısına göre seçilir" },
    { label: "Toz toplama", value: "Siklon ve filtre yüküne göre tasarlanır" },
    { label: "Sonraki proses", value: "Eleme, silo, soğutma veya paketleme" },
  ];
}

function getAirAndDustNotes(page: DrumSystemDetailPageData) {
  const lowerTitle = trText(page.title).toLocaleLowerCase("tr-TR");

  if (lowerTitle.includes("brülör")) {
    return [
      "Yakıt tipi, modülasyon oranı ve yanma odası tasarımı ısı yüküyle birlikte okunur.",
      "Doğrudan veya dolaylı ısıtma kararı ürün sıcaklık limiti ve emisyon hedefiyle birlikte verilir.",
      "Otomasyon, alev kontrolü ve emniyet ekipmanları devreye alma sürecinin ayrılmaz parçasıdır.",
    ];
  }

  if (lowerTitle.includes("fan") || lowerTitle.includes("siklon") || lowerTitle.includes("filtre")) {
    return [
      "Hava debisi ve statik basınç toplamı fan gücünü, kanal kesitlerini ve negatif basınç dengesini belirler.",
      "Siklon giriş hızı, partikül boyutu ve toz yükü birlikte değerlendirilmeden ayrıştırma performansı okunamaz.",
      "Filtre sıcaklık dayanımı, hava/kumaş oranı ve yoğuşma riski baca-emisyon performansı için kritik başlıklardır.",
    ];
  }

  return [
    "Brülör kapasitesi, buharlaştırılacak su miktarı ve ürün sıcaklık limiti birlikte hesaplanmalıdır.",
    "Fan, siklon ve jet pulse filtre hattı; hava debisi, basınç kaybı ve toz yüküne göre tek sistem olarak seçilir.",
    "Negatif basınç dengesi, yoğuşma riski, baca koşulları ve otomasyon kontrolü sahada stabil çalışmayı belirler.",
  ];
}

function getPostProcessCards(page: DrumSystemDetailPageData): DrumDetailRelatedLink[] {
  const organic = /gübre|gübre|kompost|çamur|çamur|digestat/i.test(trText(page.title));
  const biomass = /talaş|talaş|odun/i.test(trText(page.title));

  if (organic) {
    return [
      { title: "Eleme", description: "Kurutma sonrası iri-ince fraksiyon ayrımını ve recycle kararını destekler.", href: "/makinalar-ekipman/eleme-ve-siniflandirma-sistemleri" },
      { title: "Soğutma ve Tambur Sistemleri", description: "Sıcak ürünü paketleme veya depolama öncesi kontrollü hale getirir.", href: "/makinalar-ekipman/tambur-sistemleri" },
      { title: "Silo ve Bunker", description: "Nem dengesi sağlanmış ürünün tampon stok ve besleme davranışını yönetir.", href: "/makinalar-ekipman/depolama-ve-besleme-sistemleri" },
      { title: "Paketleme Sistemleri", description: "Torbalama, big-bag veya sevkiyat hazırlığını prosesin son adımına bağlar.", href: "/makinalar-ekipman/paketleme-ve-dolum-sistemleri" },
      { title: "Taşıma Ekipmanları", description: "Kurutma, eleme, silo ve paketleme arasındaki saha akışını kurar.", href: "/makinalar-ekipman/tasima-ekipmanlari" },
      { title: "Granülasyon ve Tambur Ailesi", description: "Gübre ve organik ürünlerde son form hazırlama adımlarıyla entegre edilir.", href: "/makinalar-ekipman/tambur-sistemleri" },
    ];
  }

  if (biomass) {
    return [
      { title: "Eleme", description: "Heterojen talaş ve yonga akışlarında parça boyutu kontrolü sağlar.", href: "/makinalar-ekipman/eleme-ve-siniflandirma-sistemleri" },
      { title: "Kırıcı ve Parçalayıcılar", description: "Kurutma öncesi veya sonrası boyut düzenleme ihtiyacını destekler.", href: "/makinalar-ekipman/kiricilar-ve-parcalayicilar" },
      { title: "Silo Sistemleri", description: "Peletleme veya paketleme öncesi kontrollü stoklama sağlar.", href: "/makinalar-ekipman/depolama-ve-besleme-sistemleri" },
      { title: "Taşıma Ekipmanları", description: "Hafif partikül akışlarında besleme ve transfer dengesini korur.", href: "/makinalar-ekipman/tasima-ekipmanlari" },
      { title: "Paketleme Sistemleri", description: "Kurutulmuş ürünün torbalama veya sevkiyat hazırlığını tamamlar.", href: "/makinalar-ekipman/paketleme-ve-dolum-sistemleri" },
      { title: "Toz Toplama", description: "Hafif ve yanıcı partiküller için toz yükünü kontrollü şekilde yönetir.", href: "/makinalar-ekipman/toz-toplama-sistemleri" },
    ];
  }

  return [
    { title: "Eleme Sistemleri", description: "Kurutulan mineral veya kumun tane sınıflarına ayrılmasını sağlar.", href: "/makinalar-ekipman/eleme-ve-siniflandirma-sistemleri" },
    { title: "Silo Sistemleri", description: "Düşük ve stabil nemli ürünün kontrollü stoklanmasını sağlar.", href: "/makinalar-ekipman/depolama-ve-besleme-sistemleri" },
    { title: "Paketleme ve Dolum", description: "Big-bag, torbalama veya sevkiyat hazırlığını prosesle entegre eder.", href: "/makinalar-ekipman/paketleme-ve-dolum-sistemleri" },
    { title: "Taşıma Ekipmanları", description: "Kurutma sonrası transfer hattını saha yerleşimine göre kurar.", href: "/makinalar-ekipman/tasima-ekipmanlari" },
    { title: "Bunker ve Hazneler", description: "Kurutma öncesi ve sonrası tampon stok ve kontrollü besleme sağlar.", href: "/makinalar-ekipman/depolama-ve-besleme-sistemleri" },
    { title: "Kurutma Tamburu", description: "Ana kurutma ekipmanını mekanik tasarım odağıyla inceleyin.", href: "/makinalar-ekipman/tambur-sistemleri/kurutma-tamburu" },
  ];
}

function getCalculatorFields(page: DrumSystemDetailPageData) {
  if (page.slug === "kurutma-tamburu-hesaplama") {
    return [
      "Yaş ürün kapasitesi",
      "Giriş nemi",
      "Çıkış nemi",
      ...
      "Tahmini ısı yükü",
      "Tambur hacmi",
      "Fan ve brülör ön değerlendirmesi",
    ];
  }
...
function ProjectDataGrid({ page }: { page: DrumSystemDetailPageData }) {
  return (
    <SectionCard title="Temel Proje Verileri">
      <p>
        Kesin değerler proje verisi, numune analizi veya pilot kurutma testine göre netleşir.
        Bu özet, ilk teknik görüşmede hangi bilgilerin birlikte okunacağını gösterir.
      </p>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {getProjectDataCards(page).map((item) => (
          <div
...
function AirAndDustSection({
  ...
  return (
    <section className="grid gap-6 rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm md:p-8 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#278DC0]">
          Hava ve Toz Kontrolü
        </p>
        <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950 md:text-3xl">
          {trText(
            page.slug === "kurutma-tamburunda-fan-siklon-filtre-secimi"
              ? "Fan, Siklon ve Jet Pulse Filtre Seçimi"
              : page.slug === "kurutma-tamburunda-brulor-secimi"
                ? "Brülör ve Sıcak Hava Sistemi Seçimi"
                : "Kurutma Havası ve Toz Toplama Sistemi",
          )}
        </h2>
        <div className="mt-5 space-y-4 text-sm leading-8 text-slate-700 sm:text-base">
          {getAirAndDustNotes(page).map((note) => (
            <p key={note}>{trText(note)}</p>
          ))}
        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {[
            "Hava debisi ve basınç kaybı",
            "Siklon giriş hızı ve toz yükü",
            "Filtre sıcaklık dayanımı",
            "Yakıt, otomasyon ve emisyon kontrolü",
          ].map((item) => (
            <div
              key={item}
              className="rounded-2xl border border-[#278DC0]/15 bg-[#F1F8FC] px-4 py-3 text-sm font-semibold text-[#154764]"
            >
              {trText(item)}
            </div>
          ))}
      ...
      <div className="overflow-hidden rounded-[26px] border border-slate-200 bg-slate-50">
        <div className="relative aspect-[4/5]">
          <Image
            src={heroVisual.src}
            alt={trText(`${page.title} hava ve toz toplama sistemi`)}
            fill
...
function PostProcessSection({ page }: { page: DrumSystemDetailPageData }) {
  const cards = getPostProcessCards(page);
  ...
  return (
    <section className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
      <div className="max-w-3xl">
        ...
        <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950 md:text-3xl">
          {trText(`${page.title} Sonrası Proses ve Ekipmanlar`)}
        </h2>
        <p className="mt-4 text-base leading-8 text-slate-600">
          Kurutma tamburundan çıkan ürünün eleme, soğutma, silo, paketleme veya son
          ürün hazırlama adımları ana kurutma hesabı kadar önemlidir. Bu nedenle
          kurutma hattı, sonraki ekipmanlarla birlikte planlanmalıdır.
        </p>
      </div>
...
function CalculatorCtaSection({ page }: { page: DrumSystemDetailPageData }) {
  return (
    <section className="grid gap-6 rounded-[30px] border border-[#278DC0]/20 bg-gradient-to-br from-[#F1F8FC] to-white p-6 shadow-sm md:p-8 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#278DC0]">
          Hesaplama ve Teklif
        </p>
        <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950 md:text-3xl">
          {trText(`${page.title} İçin Ön Teknik Veri Hazırlığı`)}
        </h2>
        <p className="mt-4 text-base leading-8 text-slate-600">
          Aşağıdaki bilgiler paylaşıldığında kapasite, nem yükü, tambur geometri
          aralığı, fan debisi ve brülör kapasitesi daha sağlıklı ön değerlendirilebilir.
        </p>
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          ...
          <Link
            href="/iletisim"
            className="inline-flex min-h-[48px] items-center justify-center rounded-full border border-[#278DC0] bg-white px-6 text-sm font-semibold text-[#154764] transition hover:bg-[#F1F8FC]"
          >
            Teknik Teklif Al
          </Link>
        </div>
      </div>
      <div className="rounded-[26px] border border-white bg-white p-5 shadow-[0_20px_55px_rgba(15,23,42,0.08)]">
        <h3 className="text-lg font-semibold text-slate-950">Gerekli Bilgiler</h3>
        <ul className="mt-4 space-y-3">
          {getCalculatorFields(page).map((item) => (
            <li key={item} className="flex gap-3 text-sm font-semibold leading-6 text-slate-700">
              <span className="mt-1 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#278DC0]/10 text-[11px] text-[#154764]">
                ✓
              </span>
...
export function DrumSystemDetailPage({
  ...
  const useDryingClusterTemplate = isDryingBlogClusterPage(page.slug);
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900">
      ...
      <article className="pb-16 sm:pb-20">
        <section
          className={
            useDryingClusterTemplate
              ? "border-b border-[#154764]/20 bg-gradient-to-br from-[#154764] via-[#1e6f98] to-[#278DC0]"
              : "border-b border-slate-200 bg-gradient-to-br from-white via-sky-50 to-[#eef6fb]"
          }
        >
          <div className="site-container py-10 sm:py-12 lg:py-14">
            <nav
              ...
              className={`flex flex-wrap gap-2 text-sm ${
                useDryingClusterTemplate ? "text-white/75" : "text-slate-500"
              }`}
            >
              <Link
                href="/"
              ...
              <span
                className={
                  useDryingClusterTemplate ? "font-medium text-white" : "font-medium text-slate-700"
                }
              >
                {trText(page.title)}
              </span>
            ...
            <div
              className={
                useDryingClusterTemplate
                  ? "mt-6 grid gap-8 rounded-[34px] border border-white/18 bg-white/10 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.24)] backdrop-blur md:p-8 lg:grid-cols-[minmax(0,1fr)_360px]"
                  : "mt-6 max-w-4xl rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)] md:p-8"
              }
            >
              <div>
              <div className="flex flex-wrap items-center gap-3">
                <span
                  className={
                    useDryingClusterTemplate
                      ? "inline-flex rounded-full border border-white/25 bg-white/12 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white"
                      : "inline-flex rounded-full bg-[#278DC0]/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#154764]"
                  }
                >
              ...
              <h1
                className={
                  useDryingClusterTemplate
                    ? "mt-5 text-4xl font-semibold tracking-tight text-white md:text-5xl"
                    : "mt-5 text-4xl font-semibold tracking-tight text-slate-950 md:text-5xl"
                }
              >
                {trText(page.title)}
              </h1>
              <p
                className={
                  useDryingClusterTemplate
                    ? "mt-5 max-w-3xl text-base leading-8 text-white/88 sm:text-lg"
                    : "hidden"
                }
              >
              ...
              <div
                className={
                  useDryingClusterTemplate
                    ? "mt-6 flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-[0.14em] text-white/85"
                    : "hidden"
                }
              >
              ...
              <div
                className={
                  useDryingClusterTemplate ? "mt-6 flex flex-wrap gap-3" : "hidden"
                }
              >
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <Link
                  ...
                  className={
                    useDryingClusterTemplate
                      ? "inline-flex min-h-[48px] items-center justify-center rounded-full bg-white px-6 text-sm font-semibold text-[#154764] transition hover:bg-slate-100"
                      : "inline-flex min-h-[48px] items-center justify-center rounded-full bg-[#278DC0] px-6 text-sm font-semibold text-white transition hover:bg-[#154764]"
                  }
                >
                ...
                <a
                  ...
                  className={
                    useDryingClusterTemplate
                      ? "inline-flex min-h-[48px] items-center justify-center rounded-full border border-white/55 px-6 text-sm font-semibold text-white transition hover:bg-white/12"
                      : "hidden"
                  }
                >
                ...
                {page.heroTertiaryLabel && page.heroTertiaryHref ? (
                  <Link
                    ...
                    className={
                      useDryingClusterTemplate
                        ? "inline-flex min-h-[48px] items-center justify-center rounded-full border border-white/35 px-6 text-sm font-semibold text-white transition hover:bg-white/12"
                        : "hidden"
                    }
                  >
              ...
              {page.heroImageSrc ? (
                <div
                  className={
                    useDryingClusterTemplate
                      ? "overflow-hidden rounded-[28px] border border-white/18 bg-white/10 shadow-[0_24px_60px_rgba(15,23,42,0.18)]"
                      : "hidden"
                  }
                >
        ...
        <div className="site-container mt-8">
          <div className="grid gap-8">
            ...
            {useDryingClusterTemplate ? (
              <>
                <ProjectDataGrid page={page} />
                <AirAndDustSection page={page} heroVisual={heroVisual} />
                <PostProcessSection page={page} />
              </>
            ) : null}
            ...
            {useDryingClusterTemplate ? <CalculatorCtaSection page={page} /> : null}
