...
/**
 * Bölüm içerik modeli. Verilmezse Türkçe varsayılanı kullanılır; yabancı diller
 * aynı bileşeni kendi metin ve bağlantılarıyla besler. Kart tasarımı, ikonlar ve
 * görseller tüm dillerde ortaktır (id üzerinden eşleşir).
 */
export type HomeQuickAccessContent = {
  heading: string;
  description: string;
  exploreLabel: string;
  viewAllLabel: string;
  viewAllHref: string;
  items: { id: string; title: string; href: string; alt: string }[];
};
...
export function HomeQuickAccessSection({ content }: { content?: HomeQuickAccessContent }) {
  const heading = content?.heading ?? "Sektöre Özel Çözümler";
  const description =
    content?.description ??
    "Gübre, kompost, biyogaz, maden, arıtma çamuru ve kimyasal proses uygulamalarında ihtiyaca uygun makine ve tesis çözümlerini inceleyin.";
  const exploreLabel = content?.exploreLabel ?? "Sektörü İncele";
  const viewAllLabel = content?.viewAllLabel ?? "Tüm sektörleri görüntüle";
  const viewAllHref = content?.viewAllHref ?? "/sektorler";
  const items = content
    ? content.items.flatMap((item) => {
        const base = sectorShowcaseItems.find((candidate) => candidate.id === item.id);
        return base ? [{ ...base, title: item.title, href: item.href, alt: item.alt }] : [];
      })
    : sectorShowcaseItems;
              ...
              <div className="relative z-10 flex h-full flex-col justify-between p-6 text-white">
                ...
                <div className="relative">
                  ...
                  <span className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-white/95 [text-shadow:0_6px_18px_rgba(15,23,42,0.4)]">
                    ...
                    <span
                      aria-hidden="true"
                      className="inline-block transition duration-300 group-hover:translate-x-0.5 group-focus-visible:translate-x-0.5 rtl:-scale-x-100 rtl:group-hover:-translate-x-0.5"
                    >
                      →
                    </span>
                  </span>
