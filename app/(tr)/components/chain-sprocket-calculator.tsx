...
function ResultGrid({ title, items }: { title: string; items: Array<{ label: string; value: string; tone?: Tone }> }) {
  return <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4 sm:p-5"><h3 className="text-lg font-semibold text-slate-950">{title}</h3><div className="mt-4 grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{items.map((item) => <div key={item.label} className={`rounded-2xl border px-4 py-3.5 ${item.tone === "success" ? "border-emerald-100 bg-emerald-50" : item.tone === "warning" ? "border-amber-100 bg-amber-50" : item.tone === "danger" ? "border-rose-100 bg-rose-50" : "border-slate-200 bg-white"}`}><p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{item.label}</p><p className="mt-1.5 text-base font-semibold text-slate-950">{item.value}</p></div>)}</div></div>;
}
...
function GuideBox() {
  return <InfoBox title="Nasıl Kullanılır?" text="1. Gerekli alanları doldürün. 2. Standart seçilecekse dropdown üzerinden seçin. 3. Hesapla ile sonucu güncelleyin. 4. Sonuç ve teknik uyarıları kontrol edin." />;
}
...
function CustomerSelectionCard({ children }: { children: ReactNode }) {
  return (
    <div className="rounded-3xl border border-blue-200 bg-white p-5 shadow-sm shadow-blue-100/40 sm:p-6">
      <h3 className="text-lg font-semibold text-slate-950">Müşteri Giriş Alanı</h3>
      <p className="mt-2 text-sm leading-7 text-slate-600">Gerekli seçim ve girişleri bu bölümde tek seferde doldürün.</p>
      <p className="mt-2 text-xs font-medium leading-6 text-slate-500">Lütfen gerekli seçim ve girişleri doldürün. Sonuçlar ve standart veriler otomatik güncellenecektir.</p>
      <div className="mt-4 grid gap-x-4 gap-y-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{children}</div>
    </div>
  );
}
...
function AutoFieldsCard({ children }: { children: ReactNode }) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{children}</div>
    </div>
  );
}
...
function LeadBox() {
  return <div className="rounded-3xl border border-blue-100 bg-blue-50 px-5 py-6 sm:px-6"><h3 className="text-lg font-semibold text-slate-950">Teknik özet raporu hazır</h3><p className="mt-3 text-sm leading-7 text-slate-600">Bu sonuçlar yazdırılabilir ve teklif dosyasına aktarılabilir. Zincir dişli geometrisi, oran ve sistem uygunluğu için teknik görüşme veya teklif talebi oluşturabilirsiniz.</p><div className="mt-5 flex flex-wrap gap-3"><Link href="/iletisim" className="inline-flex min-h-[44px] items-center justify-center rounded-full bg-blue-700 px-5 text-sm font-semibold text-white transition hover:bg-blue-800">Teklif Al</Link><Link href="/iletisim" className="inline-flex min-h-[44px] items-center justify-center rounded-full border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:text-blue-700">İletişime Geç</Link></div></div>;
}
